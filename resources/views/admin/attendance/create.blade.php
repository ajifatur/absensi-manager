            ...
            <div class="card-body">
                <form method="post" action="{{ route('admin.attendance.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <select name="user_id" class="form-select form-select-sm {{ $errors->has('user_id') ? 'border-danger' : '' }}" id="member" {{ (Auth::user()->role_id == role('super-admin') && old('office_id') == null) || Auth::user()->role_id == role('admin') || Auth::user()->role_id == role('manager') ? 'disabled' : '' }}>
                                @if(Auth::user()->role_id == role('super-admin'))
                                    @if(old('user_id') != null || old('office_id') != null)
                                        <option value="" selected>--Pilih--</option>
